...
class LinkedList(object):
    ...
    def printList(self, head):
        temp_head_val = head
        while temp_head_val is not None:
            print(temp_head_val.data_val)
            temp_head_val = temp_head_val.next_val

    def append_to_tail(self, node):
        temp_head_val = self.head_val
        while temp_head_val.next_val is not None:
            temp_head_val = temp_head_val.next_val
        temp_head_val.next_val = node

    def delete_a_node(self, head, node_val):
        n = head
        if n.data_val == 5:
            return head.next
